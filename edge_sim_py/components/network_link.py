...
class NetworkLink(dict, ComponentManager, Agent):
    ...
    def __init__(self, obj_id: int = None) -> object:
        ...
        # Link delay
        self["delay"] = 0
        self["original_delay"] = copy.deepcopy(self.delay )
    ...
    # def remove(self):
    #     """Method that removes the link from use in routing decisions."""
    #     self["active"] = False
    #     self["delay"] = float('inf')
    #     print(f'Link {self.id} removed.')
    ...
    def collect(self) -> dict:
        ...
        metrics = {
            "Link ID": self.id,
            "Original Delay": self.original_delay,
            "Delay": self.delay,
            "Active": self.active,
            "Failed": self.failed,
    ...
    def step(self):
        ...
        if self.model.schedule.time == 0: 
            self.original_delay = self.delay # Does not set original delay to delay until time = 1. Find another way. 
        return None 
